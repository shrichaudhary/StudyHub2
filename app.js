...
/* ============================================================================
   4 & 11. URL ROUTER & PROFESSIONAL CBT EXAM ENGINE
   ============================================================================ */

// --- USER PARAMETERS & UI CONFIGURATION ---
const CBT_COLOR_UNVISITED = "unvisited";
const CBT_COLOR_UNANSWERED = "not-answered";
const CBT_COLOR_ANSWERED = "answered";
const CBT_COLOR_MARKED = "marked";

// --- STATE EXTENSIONS ---
let questionStatus = []; // Tracks: 0=unvisited, 1=visited(unanswered), 2=answered, 3=marked

// --- VIRTUAL ROUTER (HISTORY API) ---
// This enables MPA URLs (e.g., /dashboard) without refreshing the page, keeping SPY MODE alive.
window.navigate = (path) => {
    window.history.pushState({}, "", path);
    handleRoute(path);
};

window.onpopstate = () => { handleRoute(window.location.pathname); }; // Handles browser back button
...
function handleRoute(path) {
    ...
    if (path === '/' || path === '/index.html') {
        ...
        document.getElementById('section-user-auth').classList.remove('hidden');
        window.switchAuthTab('login');
    } else if (path === '/dashboard') {
        if(currentUser) document.getElementById('section-user-dashboard').classList.remove('hidden');
        else window.navigate('/auth/login');
    } else if (path === '/admin') {
        document.getElementById('section-admin-dashboard').classList.remove('hidden');
    } else if (path.startsWith('/quizzes/')) {
        const parts = path.split('/');
        const qId = parts[2];
        if (parts.length === 3) window.initiateQuiz(qId); // /quizzes/[id] (Instructions)
        else if (parts[3] === 'attempt') window.startTestForce(qId); // /quizzes/[id]/attempt
    } else {
        // Fallback
        document.getElementById('section-home').classList.remove('hidden');
    }
...
// Override old navigation helpers to use the Router
window.goToHome = () => window.navigate('/');
window.goToAdminDashboard = () => window.navigate('/admin');


// --- CBT EXAM ENGINE LOGIC ---

window.previewQuiz = (fid) => {
    isPreviewMode = true;
    currentUser = { username: "Admin_Preview" }; 
    window.navigate(`/quizzes/${fid}`);
};

window.initiateQuiz = (fid) => {
    if(!currentUser) { alert("Please Login first!"); return window.navigate('/auth/login'); }
    ...
    document.getElementById('ins-text').innerText = currentQuiz.instructions || "No specific instructions provided.";
    ...
    // Reset checkbox
    const chk = document.getElementById('agree-rules-chk');
    if(chk) { chk.checked = false; document.getElementById('start-exam-btn').disabled = true; }

    // Update URL to match MPA Map
    window.history.pushState({}, "", `/quizzes/${fid}`);
    hideAll(); 
    document.getElementById('section-instructions').classList.remove('hidden');
};

window.startTest = () => { window.navigate(`/quizzes/${currentQuiz.firestoreId}/attempt`); };

window.startTestForce = (quizId) => {
    if(!currentQuiz || currentQuiz.firestoreId !== quizId) currentQuiz = appData.quizzes.find(q => q.firestoreId === quizId);
    
    currentQIndex = 0;
    userResponses = {};
    questionStatus = new Array(currentQuiz.questions.length).fill(0); // 0 = unvisited
    questionStatus[0] = 1; // First question visited
    quizStartTime = Date.now(); 
    
    document.getElementById('exam-title-header').innerText = currentQuiz.title;
    document.getElementById('cbt-student-name').innerText = currentUser.username;
    ...
    timerInterval = setInterval(() => {
        ...
        // Visual warning under 2 mins
        if(timeRemaining < 120 && display) display.style.backgroundColor = "#c0392b"; 
        
        if(timeRemaining<=0) { clearInterval(timerInterval); alert("Time's up! Auto-submitting."); window.submitTest(); }
    }, 1000);
...
function renderGrid() {
    ...
    currentQuiz.questions.forEach((_, i) => {
        let statusClass = CBT_COLOR_UNVISITED;
        if(questionStatus[i] === 1) statusClass = CBT_COLOR_UNANSWERED;
        if(questionStatus[i] === 2) statusClass = CBT_COLOR_ANSWERED;
        if(questionStatus[i] === 3) statusClass = CBT_COLOR_MARKED;

        grid.innerHTML += `<div id="grid-btn-${i}" class="palette-btn ${statusClass}" onclick="window.jumpToQuestion(${i})">${i+1}</div>`;
    });
}

window.jumpToQuestion = (i) => { 
    // Before jumping, assess current question status
    updateCurrentQuestionStatus();
    currentQIndex = i; 
    if(questionStatus[i] === 0) questionStatus[i] = 1; // Mark as visited if unvisited
    loadQuestion(); 
};
...
window.changeQuestion = (d) => { 
    updateCurrentQuestionStatus();
    currentQIndex += d; 
    if(questionStatus[currentQIndex] === 0) questionStatus[currentQIndex] = 1;
    loadQuestion(); 
};

window.markForReview = () => {
    questionStatus[currentQIndex] = 3; // Marked
    if(currentQIndex < currentQuiz.questions.length - 1) currentQIndex++;
    loadQuestion();
};

function updateCurrentQuestionStatus() {
    // If not marked for review, check if answered
    if (questionStatus[currentQIndex] !== 3) {
        const hasAns = userResponses[currentQIndex] !== undefined && userResponses[currentQIndex] !== null && userResponses[currentQIndex] !== "" && userResponses[currentQIndex].length !== 0;
        questionStatus[currentQIndex] = hasAns ? 2 : 1; 
    }
}

function loadQuestion() {
    renderGrid(); // Refresh palette colors
    ...
    document.getElementById('active-q-marks').innerText = q.marks || 1;
    ...
    if(q.type === 'text') {
        ...
        area.innerHTML = `<textarea class="input-field" rows="6" placeholder="Type your answer here..." onblur="window.saveTextResp(this.value)" onkeyup="window.saveTextResp(this.value)">${val}</textarea>`;
    } 
        ...
        area.innerHTML = `<p class="muted">Upload photos of your solution (Multiple allowed).</p><input type="file" accept="image/*" multiple onchange="window.handleImageUpload(this)"><div class="img-preview-box">${previewHTML}</div>`;
    }
    else {
        q.options.forEach(opt => {
            let chk = (q.type==='mcq' && userResponses[currentQIndex]===opt) || (q.type==='msq' && userResponses[currentQIndex]?.includes(opt));
            area.innerHTML += `
                <label class="option-block" style="${chk ? 'background:#eaf2f8; border-color:#2980b9;' : ''}">
                    <input type="${q.type==='mcq'?'radio':'checkbox'}" name="qOpt" value="${opt}" ${chk?'checked':''} onchange="window.saveOpt('${q.type}')">
    ...
    if(next) {
        if(currentQIndex===currentQuiz.questions.length-1) {
            next.innerText = "Save Data";
            next.classList.replace('btn-primary', 'btn-success');
        } else {
            next.innerText = "Save & Next →";
            next.classList.replace('btn-success', 'btn-primary');
        }
    }
}
...
window.saveOpt = (type) => {
    ...
    loadQuestion(); // Reload to highlight selected option box
};
